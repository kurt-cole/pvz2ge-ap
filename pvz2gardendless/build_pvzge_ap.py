...
(function() {
  ...
  const _AP_CAPTURES = {
    ...
    'levelController': function(v) {
      window._AP_levelController = v; installConveyorHook(v); installBudgetHook(v);
    },
    ...
    switch(pkt.cmd){
      ...
      case 'Connected':
        ...
        if(pkt.slot_data){
          ...
          // Under the budget zombie roll the tier shuffle stands down: the
          // roll rewrites the level itself, and swapping its result again
          // would field zombies generation never planned for.
          st.zombieBudgetRoll = !!pkt.slot_data.zombie_budget_roll;
          st.zombieBudget     = pkt.slot_data.zombie_budget || null;
          st.shuffleZombies = !!pkt.slot_data.shuffle_zombies && !st.zombieBudgetRoll;
