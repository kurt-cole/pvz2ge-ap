...
Phase 1 of .claude/ZOMBIE_RANDO_DEV2.md. Nothing in the world imports it yet.
...
class Level:
    ...
    def _group(self, kind, alias, data):
        ...
        if kind in ("jittered", "ground", "storm", "grave_spawn", "qigong"):
            ...
            out["f"] = "SwashbucklerCount"
        elif kind == "beach":
            out["z"] = [[data.get("ZombieName"), as_int(data.get("ZombieCount"))]]
            out["f"] = "ZombieCount"
        elif kind in ("spider", "parachute"):
            counts = Counter({data.get("SpiderZombieName"): as_int(data.get("SpiderCount"))})
            if kind == "parachute":
                brought, _ = self._list_entries(data.get("ZombiesToBringWith"))
                counts.update(brought)
            out["z"] = sorted([n, c] for n, c in counts.items() if n)
            out["f"] = "SpiderCount"
        elif kind == "dropship":
