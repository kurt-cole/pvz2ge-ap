...
def render(plants, excluded, required, rungs, summary, sun_costs, consumables):
    ...
    for name in sorted(sun_costs):
        out.append(f"    {name!r}: {sun_costs[name]},\n")
    out.append("}\n")
...
def main() -> int:
    ...
    sun_costs = {name: int(game.sun_cost(cn)) for name, cn in sorted(bridge.items())
                 if game.sun_cost(cn) is not None}
    consumables = sorted(single_use & set(bridge))
    text = render(plants, excluded, required, rungs, summary, sun_costs, consumables)
    blob = {
